...
- **i18n**: `data.json` only carries the English names (CDragon's `default`
  ...
  extra bandwidth.
- **PWA**: `sw.js` treats every same-origin GET as network-first — the app shell
  (HTML/CSS/JS/icons) as well as `data.json` / `i18n/*.json`. The cache is purely
  an offline fallback, so an edit or a language switch never needs a second
  reload to show up. Splash images from CDragon are not cached, so full offline
  use (with images) is not supported.
...
│   ├── hero.js                      #   home hero band (newest splashes, rotating)
│   ├── zip.js                       #   bulk ZIP download (JSZip)
│   ├── lightbox.js                  #   fullscreen viewer + (in-app) slideshow
│   ├── tutorial.js                  #   first-visit walkthrough (? re-opens it)
│   ├── share.js                     #   share the site (Web Share API / clipboard)
...
notes (changelog) are auto-generated by GitHub as one flat list of the PRs merged
since the previous tag (`.github/release.yml` only filters out bot authors and
anything labelled `ignore-for-release`), so there's no hand-maintained
`CHANGELOG.md` to keep in sync. To build one yourself:
