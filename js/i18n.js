...
export const UI_STRINGS = {
  default: {
    ...
    handoff_body: "This sends your gallery to the OpenLeagueDisplay desktop app so you can set them as wallpaper in one click. On Windows the app is launched by the link itself (it just has to be installed); elsewhere it has to be running already. Neither? Export to a file instead and import it there.",
    handoff_open: "Send to desktop app",
    handoff_export: "Export to a file instead",
    handoff_launching: "Launching the desktop app… Nothing happening? It may not be installed on this PC.",
  ...
  ja_jp: {
    ...
    handoff_body: "ギャラリーを OpenLeagueDisplay デスクトップ版に送り、1クリックで壁紙にできます。Windows ならリンクからデスクトップ版が起動します（インストール済みであれば起動中でなくて OK）。それ以外の OS では起動中である必要があります。どちらでもなければ、代わりにファイルへ書き出して向こうで読み込んでください。",
    handoff_open: "デスクトップ版に送る",
    handoff_export: "代わりにファイルに書き出す",
    handoff_launching: "デスクトップ版を起動しています… 反応がない場合、このPCにインストールされていない可能性があります。",
